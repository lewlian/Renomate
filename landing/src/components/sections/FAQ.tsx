...
export function FAQ() {
  return (
    <section id="faq" className="py-24 md:py-32 bg-mint-bg">
      <div className="container-narrow">
        <div className="mb-10">
          ...
          <ScrollReveal delay={0.05}>
            <h2 className="font-heading text-display-lg text-ink">Questions, answered.</h2>
          </ScrollReveal>
        </div>
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <details
      className="border-b border-cloud py-5 group"
      open={open}
    ...
    >
      <summary className="flex justify-between items-center gap-4 cursor-pointer list-none">
        ...
        <span
          className={`font-heading text-[24px] text-slate transition-transform duration-200 ease-brand ${
            open ? "rotate-45" : ""
          }`}
        >
          +
